...
import { isWaitlistOnly, marketingSignupPath } from '../../../lib/waitlistOnly';
...
export default function HeroQrOverlay() {
  ...
  useEffect(() => {
    queueMicrotask(() => {
      ...
      setQrUrl(`${getPublicSiteOrigin()}${marketingSignupPath()}`);
      setVisible(true);
    });
  }, []);
  ...
  return (
    <div
      ...
      aria-label={
        waitlist
          ? 'Lista de espera en el celular con código QR'
          : 'Registro móvil con código QR'
      }
    >
      <div className="flex flex-col items-center gap-2">
        ...
        <p className="text-center text-[11px] font-semibold leading-snug text-gray-900 sm:text-xs">
          {waitlist
            ? 'Escaneá para anotarte en la lista de espera en tu celular.'
